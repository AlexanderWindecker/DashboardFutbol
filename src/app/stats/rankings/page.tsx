...
export default async function RankingsPage({ searchParams }: { searchParams: { type: string, pos?: string, filter?: string, seasonId?: string } }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-900/40 backdrop-blur-md border border-slate-800 rounded-2xl overflow-x-auto shadow-2xl">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center gap-4">
                                        <div className="relative group-hover:scale-110 transition-transform">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center text-slate-400 font-black border border-slate-700 shadow-lg">
                                                {p.name.charAt(0)}
                                            </div>
                                            {p.isVacation && (
                                                <div className="absolute -top-1.5 -right-1.5 bg-amber-500 rounded-lg p-1 border-2 border-slate-950 shadow-lg">
                                                    <Palmtree size={10} className="text-white" />
                                                </div>
                                            )}
                                            {p.isInjured && (
                                                <div className="absolute -top-1.5 -right-1.5 bg-red-600 rounded-lg p-1 border-2 border-slate-950 shadow-lg animate-pulse">
                                                    <Plus size={10} className="text-white rotate-45 stroke-[4px]" />
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="font-black text-slate-100 group-hover:text-white transition-colors text-base tracking-tight leading-tight">{p.name}</span>
                                        </div>
                                    </div>
                                </td>
                                {isSkillsRanking && (
                                    <>
                                        <td className="p-4 border-l border-slate-800/30 bg-sky-500/[0.02]">
                                            <SkillBar
                                                label={p.isUsingGoalkeeperStats ? "REF" : "RIT"}
                                                value={p.skillDetails.pac}
                                                colorClass="bg-sky-400 shadow-sky-500/50"
                                            />
                                        </td>
                                        <td className="p-4 bg-orange-500/[0.02]">
                                            <SkillBar
                                                label={p.isUsingGoalkeeperStats ? "UBI" : "TIR"}
                                                value={p.skillDetails.sho}
                                                colorClass="bg-orange-400 shadow-orange-500/50"
                                            />
                                        </td>
                                        <td className="p-4 bg-emerald-500/[0.02]">
                                            <SkillBar
                                                label={p.isUsingGoalkeeperStats ? "DEF" : "PAS"}
                                                value={p.skillDetails.pas}
                                                colorClass="bg-emerald-400 shadow-emerald-500/50"
                                            />
                                        </td>
                                        <td className="p-4 bg-purple-500/[0.02]">
                                            <SkillBar
                                                label={p.isUsingGoalkeeperStats ? "SAQ" : "REG"}
                                                value={p.skillDetails.dri}
                                                colorClass="bg-purple-400 shadow-purple-500/50"
                                            />
                                        </td>
                                        <td className="p-4 bg-rose-500/[0.02]">
                                            <SkillBar
                                                label={p.isUsingGoalkeeperStats ? "SEG" : "VEL"}
